...
import { auth, provider, githubProvider, db } from './firebase'; // el santísimo trinity (ahora cuarteto) de Firebase
...
// Login con GitHub: Para los verdaderos amantes de la terminal y los commits nocturnos
const loginGithub = async () => {
  authError.value = '';
  authLoading.value = true; // Spinner: "Firebase sigue pensando, ahora en modo octocat 🐱"
  
  try {
    const result = await signInWithPopup(auth, githubProvider);
    const userObj = result.user;
    
    // Inicialización en caliente (tu salvavidas por si Firestore no tiene el perfil listo)
    const profileRef = doc(db, 'profiles', userObj.uid);
    const profileSnap = await getDoc(profileRef);
    
    if (!profileSnap.exists()) {
      // Si es un alma nueva que viene de GitHub, le creamos su documento
      await setDoc(profileRef, {
        displayName: userObj.displayName || userObj.reloadUserInfo?.screenName || 'Tuxero Anónimo',
        photoURL: userObj.photoURL || '',
        bio: 'Desarrollador que sobrevivió a los reboots de producción.',
        createdAt: new Date() // El nacimiento de un alma geek
      });
    }
    
    showAuthModal.value = false; // Cerramos el modal de una vez, misión cumplida.
  } catch (error) {
    console.error("Error en login de GitHub:", error);
    // Un mensaje de vergüenza personalizado para los fallos de OAuth
    authError.value = "Hubo un problema con GitHub. Tal vez tu clave SSH está celosa. Intenta de nuevo.";
  } finally {
    ...
    <transition name="expand">
      <div v-if="showAuthModal" class="modal-overlay" @click.self="showAuthModal=false">
        <div class="auth-modal">
          ...
          <button class="github-btn" @click="loginGithub" :disabled="authLoading" style="margin-top: 8px;">
            <svg width="18" height="18" viewBox="0 0 24 24"><path fill="currentColor" d="M12 2A10 10 0 0 0 2 12c0 4.42 2.87 8.17 6.84 9.5.5.08.66-.23.66-.5v-1.69c-2.77.6-3.36-1.34-3.36-1.34-.46-1.16-1.11-1.47-1.11-1.47-.9-.62.07-.6.07-.6 1 .07 1.53 1.03 1.53 1.03.9 1.52 2.34 1.07 2.91.83.1-.65.35-1.09.63-1.34-2.22-.25-4.55-1.11-4.55-4.92 0-1.11.38-2 1.03-2.71-.1-.25-.45-1.29.1-2.64 0 0 .84-.27 2.75 1.02.79-.22 1.65-.33 2.5-.33.85 0 1.71.11 2.5.33 1.91-1.29 2.75-1.02 2.75-1.02.55 1.35.2 2.39.1 2.64.65.71 1.03 1.6 1.03 2.71 0 3.82-2.34 4.66-4.57 4.91.36.31.69.92.69 1.85V21c0 .27.16.59.67.5C19.14 20.16 22 16.42 22 12A10 10 0 0 0 12 2z"/></svg>
...
.github-btn {
  ...
  width: 100%;
  padding: 11px;
  background: #24292e; /* El clásico gris oscuro de GitHub */
